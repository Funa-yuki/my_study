...
class FuncFixer(object):
    def __init__(self):
        self.funcs = {}
        self.fixers_in_framework = default_fixers.fixers
    ...
    #func_namesにしてリストを回して変更する
    #default_fixers
    #funcがない時のエラーハンドリングも必要
    def fix(self, func_name=None):
        def decorator(callback):
            ...
            def wrapper(request):
                ...
                for fixer in self.fixers_in_framework:
                    callback_ast = fixer(callback_ast)
                # アプリケーション開発者の修正
                if self.funcs.get(func_name):
