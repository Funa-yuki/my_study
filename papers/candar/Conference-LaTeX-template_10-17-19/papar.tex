...
\IEEEauthorblockA{\textit{Graduate School of Information Electrical Engineering} \\
...
%This paper provides a web application framework which has features that analyzes the source code implemented by web application developers and detects vulnerability in the source code dynamically.
%Previous methods can detect vulnerability attacks limitedly because the methods do not use information in the web application.
%The function of web application framework  we propose analyzes source code of a web application and modifies it, if necessary, to detect vulnerability attacks which is been difficult to detect.
%Our implementation and experiments show that it is possible to detect attacks against authentication leaks and SQL injection attacks using dynamic queries, which have been difficult to detect.
\end{abstract}
...
\end{lstlisting}
%In the code above, the first line(Code \ref{code:decorator}) is the decorator that stores the callback function in the framework, and the second line and subsequent access function is the callback function that is stored.
%The argument of access() function, named request is a dictionary data which is stored request parameter.
%The line 3 to 5(Code \ref{code:decorator}) are the preparations for using SQL.
%From line 6 to 8, parameters are fetched from the request form, and lines 9 and 11 use those forms to build the SQL query.
...
%The cur.execute() method on the 12th and 16th lines is a method to have the query executed in the database.
%The cur method on the 12th line is executed when the select command is executed.
%The 13th line's cur.fetchone method returns the result of executed the query by the database.
%In this result, The second element of variable data is the telephone number, and third element of variable data is the email address.
%Then, at line 15, this callback function use the template function ``tmpl'' to return the action and the phone number and email address retrieved from the database to the user.
%This source code is vulnerable in that the query is created by directly using the request parameters at lines 9 and 11.
%The vulnerability was addressed with the following vulnerability handling function.
...
%Explaining the above code, lines 2 to 8 are the vulnerability handling functions that modify the node, and the decorator on line 1 stores this function in our framework.
%The class on and after line 10 searches and modify the callback function node on line 6.
%The 12th visit\_Call method is called when the node class name is Call.
%Nodes with the class name Call are function and method nodes.
%Lines 14 to 17 describe the conditions of the node to be modified and the modification details.
%In this implementation, when the cur.execute method is called, the argument is modified and changed to the execution result of the function escape\_special\_query.
%The escape\_special\_query function deletes a query as an invalid query when the query instruction is drop and when the where clause is commented out in the select instruction.
%This makes these instructions unavailable to the user.
...
%In the callback function called do\_login, it is required that id is ``admin'' and password is ``admin\_pass'' as a condition to return ``ADMIN\_PAGE''.
%On the other hand, the path ``/home'' is vulnerable in that the all application users can see it.
%We addressed the vulnerability with following source code.
...
%First, in the for loop from line 5 to line 16, this function find the node of the callback function that uses the is\_admin function.
%Callback function nodes that use the is \_admin function are detected by the search \_from \_is \_admin function and stored in return \_nodes \_from \_is \_admin.
%The is\_admin function is a helper provided by the framework and returns True if the ID and password match, False otherwise.
%From line 18 onwards, the nodes stored in return \_nodes \_from \_is \_admin that are not conditioned to the is \_admin function are changed to nodes that use the is \_admin function as a condition.
